...
class HotelProductCarousel extends PureComponent {
  state = {
  ...
  render() {
    ...
    return (
      <div>
        {
          hotDealReducer.hotel.items.length > 0 ? 
          <div className='promo-wrap content-padding ongoing-promo'>
            <div className='app-container'>
              {this.getTitle()}
              <Carousel items={hotDealReducer.hotel.items.length > 0 ? hotDealReducer.hotel.items : null}/>
            </div>
          </div> 
          :
...
export default connect(mapStateToProps, null)(HotelProductCarousel);