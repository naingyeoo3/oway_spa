...
class BusProductCarousel extends PureComponent {
  state = {
  ...
  render() {
    ...
    return (
      <div>
        {
          hotDealReducer.bus.items.length > 0 ?
          <div className='promo-wrap content-padding ongoing-promo'>
            <div className='app-container'>
              {this.getTitle()}
              <Carousel items={hotDealReducer.bus.items.length > 0 ? hotDealReducer.bus.items : null}/>
            </div>
          </div> 
          :
...
export default connect(mapStateToProps, null)(BusProductCarousel);