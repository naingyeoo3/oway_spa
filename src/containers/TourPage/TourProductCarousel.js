...
class TourProductCarousel extends PureComponent {
  state = {
  ...
  render() {
    ...
    return (
      <div>
        {
          hotDealReducer.tour.items.length > 0 ?
          <div className='promo-wrap content-padding ongoing-promo'>
            <div className='app-container'>
              {this.getTitle()}  
              <Carousel items={hotDealReducer.tour.items.length > 0 ? hotDealReducer.tour.items : null}/>
            </div>
          </div> 
          :
...
export default connect(mapStateToProps, null)(TourProductCarousel);