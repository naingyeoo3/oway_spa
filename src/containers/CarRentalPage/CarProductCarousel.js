...
class CarProductCarousel extends PureComponent {
  state = {
  ...
  render() {
    ...
    return (
      <div>
        {
          hotDealReducer.car.items.length > 0 ?
          <div className='promo-wrap content-padding ongoing-promo'>
            <div className='app-container'>
                {this.getTitle()}        
                <Carousel items={hotDealReducer.car.items.length > 0 ? hotDealReducer.car.items : null}/>
            </div>
          </div> 
          :
...
export default connect(mapStateToProps, null)(CarProductCarousel);