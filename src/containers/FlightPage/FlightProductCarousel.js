...
class FlightProductCarousel extends PureComponent {
  state = {
  ...
  render() {
    ...
    return (
      <div>
        {
          hotDealReducer.flight.items.length > 0 ?
          <div className='promo-wrap content-padding ongoing-promo'>
            <div className='app-container'>
              {this.getTitle()}
              <Carousel items={hotDealReducer.flight.items.length > 0 ? hotDealReducer.flight.items : null}/>
            </div>
          </div> 
          :
...
export default connect(mapStateToProps, null)(FlightProductCarousel);